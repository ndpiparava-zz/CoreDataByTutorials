...
class DataMigrationManager {
  ...
  init(modelNamed: String, enableMigration: Bool = false) {
  ...
  func performMigration() {
    ...
    if let storeModel = self.storeModel {
      if storeModel.isVersion1 {
        ...
        migrateStoreAt(URL: storeURL,
                       fromModel: storeModel,toModel: destinationModel,
...
extension NSManagedObjectModel {
  ...
  private class func moderlURLs (in modelFolder: String) -> [URL] {
  ...
  class func modelVersionsFor(modelNamed modelName: String) -> [NSManagedObjectModel] {
    return moderlURLs(in: modelName).flatMap(NSManagedObjectModel.init)
  }
  
  class func uncloudNotesModel (named modelName: String) -> NSManagedObjectModel {
    
    let model = moderlURLs(in: "UnCloudNotesDataModel")
...
func == (firstMode: NSManagedObjectModel, otherModel: NSManagedObjectModel) -> Bool {
  return firstMode.entitiesByName == otherModel.entitiesByName
}
